...
*   **AI Provider Layer:** Supports dual AI providers (OpenAI `gpt-5.2` or Gemini Pro `gemini-2.5-pro`) with built-in retry logic and **token continuation loop**. When `finish_reason=length`/`MAX_TOKENS` is detected, the system enters a multi-turn continuation loop (up to 4 continuations) that feeds partial output back and requests the LLM to continue from the exact cutoff point. Accumulated content replaces the model turn each iteration to maintain valid conversation structure. For `json_object` response format, the combined output is validated as JSON; for free-text responses (e.g., refinement), the raw concatenation is returned. Default output token limits: Gemini 65,536, OpenAI 16,384. Agent-specific limits: Architect 32K, Backend/Frontend 65K, Security 32K, Verification 16K.
*   **Orphan Recovery:** Automatically restarts AI processing for projects stuck in `planning`, `generating`, or `validating` status due to server interruptions. Recovery runs sequentially with `await` (not fire-and-forget), each project wrapped in try/catch — failures update the project status to `failed` with error message rather than creating zombie processes.
